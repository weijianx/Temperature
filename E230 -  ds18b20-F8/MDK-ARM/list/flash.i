...
# 41 "../BSP/Src/flash.c" 2


uint32_t bak[128];
static uint32_t Flash_Buf[1024];
...
{
 uint32_t buf;
 buf = *(volatile uint32_t *)read_addr;
    return buf;
}
# 98 "../BSP/Src/flash.c"
uint8_t Flash_Write_OneByte(uint32_t RWAddr, uint8_t WrData)
{
    ...
    WrAddr = (RWAddr / 1024) * 1024;
    flash_read_multi(WrAddr, buf, 1024);
    buf[RWAddr % 1024] = WrData;

 fmc_unlock();
    fmc_flag_clear(((uint32_t)((uint32_t)0x01U<<(2)))|((uint32_t)((uint32_t)0x01U<<(3)))|((uint32_t)((uint32_t)0x01U<<(4)))|((uint32_t)((uint32_t)0x01U<<(5))));

    if(buf[RWAddr % 1024] != 0xFF)
    {
  fmc_page_erase(RWAddr);
    }

    for(i = 0; i < 1024;)
    {
  fmc_word_program((WrAddr + i), (uint32_t)Flash_Buf[i/4]);
  i+=4;
 }

    fmc_lock();
 if( (*(volatile uint8_t *)RWAddr) != WrData)
  return 0;
 return 1;
}
# 137 "../BSP/Src/flash.c"
uint8_t Flash_Write_MultiBytes(uint32_t RWAddr, uint8_t const *pWrbuf, uint16_t Wrlen)
{
 ...
 if( (*(volatile uint8_t *)RWAddr) != pWrbuf[0])
  return 0;
 if(Wrlen1 != 0)
 {
...
_Bool GDFLASHOP_EraseSpecifiedFlashPages( uint32_t startaddr, uint16_t npages )
{
    ...
    if (startaddr + npages * 1024 > 0x08020000)
        return 0;


    fmc_unlock();
 fmc_flag_clear(((uint32_t)((uint32_t)0x01U<<(5))));
    fmc_flag_clear(((uint32_t)((uint32_t)0x01U<<(4))));
    fmc_flag_clear(((uint32_t)((uint32_t)0x01U<<(2))));
...
# 278 "../BSP/Src/flash.c"
_Bool GDFLASHOP_WriteData2SpecifiedFlash( uint32_t startaddr, uint32_t *psrc, uint16_t wdlen )
{
 if ((psrc == 0) || (!wdlen))
        return 0;

    if (startaddr + wdlen * 4 > 0x0801FFFF)
        return 0;
...
# 309 "../BSP/Src/flash.c"
_Bool GDFLASHOP_ReadDataFromChipFlash( uint8_t* pdest, uint32_t op_addr, uint32_t dwbufsize, uint32_t dwlen )
{
 uint32_t val;
    ...
    if (op_addr < 0x08003800)
        return 0;

    while (dwlen --)
    {
        val = *(uint32_t*)op_addr;
        *pdest ++ = (uint8_t)val;
        *pdest ++ = (uint8_t)(val >> 8);
        *pdest ++ = (uint8_t)(val >> 16);
        *pdest ++ = (uint8_t)(val >> 24);
        op_addr += 4;
    }
    return 1;
}
