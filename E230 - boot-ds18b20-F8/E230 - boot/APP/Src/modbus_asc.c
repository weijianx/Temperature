...
uint8_t const SensorSoftVersion[10] = {0x09, 'S', 'V', '1', '.', '0', '.', '0', 'G', 'D'};      //软件版本  20200413
//**************************************************************************************************
...
void MBASC_Fun41(void)
{
    ...
    SendLen = 0;
    SendBuf[SendLen++] = UARTx_RXBuff[1] ? UserPara.SlaveAddr : 0x00;
...
//    {
        if(2 == MODBUS_ASCII_RecvData(UARTx_RXBuff, &RecvLen))//校验错误
        {
            return;
        }  
        if(RecvLen <= 0)
        ...
        else
        {
            switch (UARTx_RXBuff[1])
            {
              ...
              default:
                SendLen = 0;
                SendBuf[SendLen++] = UARTx_RXBuff[0];//MBASC_GetSlaveAddr(UARTx_RXBuff);
